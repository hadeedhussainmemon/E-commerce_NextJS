      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'BreadcrumbList',
            itemListElement: [
              { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://www.petalpluspup.com/' },
              { '@type': 'ListItem', position: 2, name: 'Categories', item: 'https://www.petalpluspup.com/categories' }
            ]
          })
        }} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-px bg-gray-100 border border-gray-100">
          {visibleCategories.map((category, idx) => (
            <Link
              ...
              className="group bg-white p-12 flex flex-col items-center justify-center text-center hover:bg-gray-50 transition-colors"
            >
              <div className="w-16 h-16 rounded-full bg-gray-50 flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500">
                <CategoryIcon name={category.name} />
              </div>
              <h3 className="font-fashion-sans text-[11px] font-bold uppercase tracking-[0.3em] text-black mb-2">
                {category.name}
              </h3>
              {typeof category.count === 'number' && (
                <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">{category.count} Items</span>
              )}
