...
import GlobalSearchOverlay from "@/components/store/Search/GlobalSearchOverlay";
          ...
          <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between relative">
            ...
            <div className="hidden lg:flex items-center gap-8 flex-1">
              {leftNav.map((link) => (
                <Magnetic key={link.label}>
                  <Link
                    href={link.href}
                    className="text-[11px] font-bold uppercase tracking-[0.15em] text-gray-600 hover:text-black transition-colors px-2 py-1"
                  >
                    {link.label}
                  </Link>
                </Magnetic>
            ...
            <div className="absolute left-1/2 -translate-x-1/2 flex items-center h-full">
              <Link href="/" className="flex items-center">
                <div className="relative h-8 w-48 transition-transform duration-500 hover:scale-105 active:scale-95">
                  <Image
                    src={LOGO_PATH}
                    alt={config.appName}
                    fill
            ...
            <div className="flex items-center justify-end gap-6 flex-1 h-full">
              ...
              <div className="flex items-center gap-2">
                ...
                <Magnetic>
                  <Link href="/wishlist" className="p-2 text-gray-800 hover:text-black transition-colors relative hidden sm:block">
                    <Heart size={20} strokeWidth={1.5} />
                    {wishlistCount > 0 && (
                      <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-black text-white text-[8px] font-bold rounded-full flex items-center justify-center">
                        {wishlistCount}
                      </span>
                    )}
                  </Link>
            ...
            >
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                <div className="relative h-6 w-36">
                  <Image src={LOGO_PATH} alt={config.appName} fill className="object-contain" />
                </div>
