...
export default function CheckoutForm({ onBack, onSuccess, appliedCoupon }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ⚡ New: Use Server Action instead of internal fetch
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-12">
          ...
          <section className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-10 border-t border-gray-100 flex justify-between items-baseline">
                <span className="text-[11px] font-bold uppercase tracking-[0.4em] text-black">Final Statement</span>
                <div className="text-right">
                  <span className="text-5xl font-fashion-serif italic font-black text-black tracking-tighter">
                    {config.currency.symbol} {total.toLocaleString()}
                  </span>
                  <p className="text-[9px] font-bold uppercase tracking-widest text-gray-300 mt-2">Method: Cash On Delivery</p>
                </div>
              </div>
            </div>
          </section>
          ...
          {/* Final Submit */}
          <div className="sticky bottom-0 bg-white -mx-8 px-8 py-10 border-t border-gray-100 flex flex-col items-center gap-6">
            <Magnetic>
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full max-w-sm py-6 bg-black text-white text-[11px] font-bold uppercase tracking-[0.4em] hover:bg-gray-900 transition-colors disabled:opacity-50 flex items-center justify-center gap-6 active:scale-[0.99] shadow-2xl shadow-black/20"
              >
                {isSubmitting ? (
                  <>
            ...
            <Magnetic>
              <button
                type="button"
                onClick={onBack}
                className="text-[10px] font-bold uppercase tracking-[0.4em] text-gray-300 hover:text-black transition-colors"
              >
                Return To Selection
              </button>
            </Magnetic>
