...
import { triggerWishlistAnimation } from '@/components/common/UI/FloatingHearts';
...
import { NewBadge, SaleBadge, OutOfStockBadge } from '@/components/common/UI/ProductBadges';
...
const ProductCard = (inputProps) => {
    ...
    return (
        <div className="group relative flex flex-col bg-white overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)]">
            {/* Image Section */}
            <div className="relative aspect-[3/4] overflow-hidden bg-gray-50 shimmer-overlay">
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out hidden md:block">
                    <button
                        ...
                        className="w-full glass text-black text-[10px] font-bold uppercase tracking-[0.3em] py-4 shadow-xl hover:bg-black hover:text-white transition-all duration-500"
                    >
                        Quick View
                    </button>
                </div>
            ...
            {/* Content Section */}
            <div className="pt-6 pb-8 px-2 text-center">
                <div className="text-[9px] uppercase tracking-[0.4em] text-gray-400 mb-2.5 font-black italic font-fashion-serif">
                    {Array.isArray(category) ? category[0] : category}
                </div>
                <Link href={`/product/${slug || id}`}>
                    <h3 className="font-fashion-sans text-[14px] font-bold tracking-tight text-gray-900 group-hover:text-gray-400 transition-colors mb-3 line-clamp-1 uppercase">
                        {title}
                    </h3>
                </Link>
