...
export function formatOrderConfirmationMessage(order) {
  ...
  const trackUrl = `${config.api.baseUrl}/track-order/${order.id}`;

  const itemsList = (order.items || []).map(i => `• ${i.title} (x${i.quantity})`).join('\n');

  return `📦 ${config.appName} — Order Confirmation Required

Hello ${customerName},
Thank you for placing an order with ${config.appName}! Before we proceed, please confirm the details below:
...
export function formatOrderThankYouMessage(order) {
  ...
  return `✨ ${config.appName} — Order Confirmed
...
Thank you for shopping with ${config.appName}!`.trim();
}
